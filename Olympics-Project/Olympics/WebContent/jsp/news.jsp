...
%>
    <tr>
     <td><%= vo.getId() %></td>
     <td width = "500" onclick="location.href='/Olympics/news.do?action=read&newsid=<%= vo.getId() %>'" ><%= vo.getTitle() %></td>
...
<sr><button class="btn btn-info btn-md" onclick = "location.href='/Olympics/news.do'">목록</button></sr>
<st><button class="btn btn-info btn-md" onclick = "show('write')">글쓰기</button></st>
<div id="write" style='padding:30px'>
 <form action="/Olympics/news.do" method="post" > 
...
<div id="show" style='padding:50px'>
 <form action="/Olympics/news.do" method="post">
  ...
  <input type="button" value="삭제" onclick="location.href='/Olympics/news.do?action=delete&newsid=${ vo.id }'">
